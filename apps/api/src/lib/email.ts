  ...
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "ConcertRide <no-reply@concertride.es>",
        to: email,
        subject: "Restablece tu contraseña en ConcertRide",
        html: renderResetEmail(url),
      }),
    });
    if (!res.ok) {
      const text = await res.text().catch(() => "");
...
<html><body style="margin:0;padding:40px;background:#080808;color:#f5f5f5;font-family:-apple-system,'Inter',sans-serif;">
  <div style="max-width:480px;margin:0 auto;">
    ...
    <h1 style="font-family:Georgia,serif;font-size:36px;line-height:1.05;margin:16px 0 8px 0;letter-spacing:-1px;">Restablece tu contraseña.</h1>
    <p style="color:#888;margin:0 0 32px 0;">Pulsa el botón para crear una nueva contraseña. El enlace expira en 30 minutos y solo funciona una vez.</p>
    <a href="${url}" style="display:inline-block;background:#DBFF00;color:#000;padding:16px 28px;font-weight:700;text-transform:uppercase;letter-spacing:2px;text-decoration:none;border:2px solid #000;font-size:13px;">Cambiar contraseña</a>
    <p style="color:#666;font-size:12px;margin-top:40px;word-break:break-all;">o copia esta URL: ${url}</p>
    <p style="color:#444;font-size:11px;margin-top:32px;">Si no pediste restablecer tu contraseña, ignora este correo.</p>
  </div>
</body></html>`;
}
