...
export function FactDensityCallout({ facts, heading = "Datos clave", className }: Props) {
  ...
  return (
    ...
    >
      <p className="font-sans text-[11px] uppercase tracking-[0.18em] text-cr-primary">
        ⚡ {heading}
      </p>
      <dl className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
        {facts.map((f) => (
          <div key={f.label} data-cite={`${f.label}:${f.value}`} className="border-l-2 border-cr-primary/40 pl-3">
            <dt className="font-sans text-[11px] uppercase tracking-[0.14em] text-cr-text-muted">
              {f.label}
            </dt>
            ...
            {f.detail && (
              <p className="mt-0.5 font-sans text-[11px] uppercase tracking-[0.1em] text-cr-text-dim">
                {f.detail}
              </p>
            )}
          </div>
